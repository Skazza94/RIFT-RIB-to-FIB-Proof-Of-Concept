...
def test_add_default_route():
    rib = Rib()
    default_route = Route(default_prefix, S_SPF, default_next_hops)
    ...
    assert rib.fib.routes[default_prefix] == default_route
...
def test_add_negative_disaggregation():
    ...
    default_route = Route(default_prefix, S_SPF, default_next_hops)
    rib.put_route(default_route)
    neg_route = Route(first_negative_disagg_prefix, S_SPF, [], first_negative_disagg_next_hops)
...
def test_add_two_negative_disaggregation():
    ...
    default_route = Route(default_prefix, S_SPF, default_next_hops)
    rib.put_route(default_route)
    first_neg_route = Route(first_negative_disagg_prefix, S_SPF, [], first_negative_disagg_next_hops)
    rib.put_route(first_neg_route)
    second_neg_route = Route(second_negative_disagg_prefix, S_SPF, [], second_negative_disagg_next_hops)
...
def test_remove_default_next_hop():
    ...
    second_neg_route = Route(second_negative_disagg_prefix, S_SPF, [], second_negative_disagg_next_hops)
    rib.put_route(second_neg_route)
    default_route_fail = Route(default_prefix, S_SPF, ['S1', 'S3', 'S4'])
...
def test_add_subnet_disagg_to_first_negative_disagg():
    ...
    first_neg_route = Route(first_negative_disagg_prefix, S_SPF, [], first_negative_disagg_next_hops)
    rib.put_route(first_neg_route)
    subnet_neg_route = Route(subnet_disagg_prefix, S_SPF, [], subnet_negative_disagg_next_hops)
...
def test_remove_default_next_hop_with_subnet_disagg():
    ...
    subnet_neg_route = Route(subnet_disagg_prefix, S_SPF, [], subnet_negative_disagg_next_hops)
    rib.put_route(subnet_neg_route)
    default_route_fail = Route(default_prefix, S_SPF, ['S1', 'S2', 'S4'])
    rib.put_route(default_route_fail)
    # Test for default
    assert rib.destinations.get(default_prefix).best_route == default_route_fail
...
def test_recover_default_next_hop_with_subnet_disagg():
    ...
    default_route_fail = Route(default_prefix, S_SPF, ['S1', 'S2', 'S4'])
    rib.put_route(default_route_fail)
    default_route_recover = Route(default_prefix, S_SPF, ['S1', 'S2', 'S3', 'S4'])
    ...
    assert rib.destinations.get(default_prefix).best_route.negative_next_hops == set()
    assert rib.destinations.get(default_prefix).best_route.next_hops == {'S1', 'S2', 'S3', 'S4'}
    assert rib.fib.routes[default_prefix] == default_route_recover
...
def test_neg_disagg_fib_unreachable():
    ...
    default_route = Route(default_prefix, S_SPF, default_next_hops)
    rib.put_route(default_route)
    unreachable_route = Route(unreachable_prefix, S_SPF, [], unreachable_negative_next_hops)
...
def test_neg_disagg_fib_unreachable_recover():
    ...
    unreachable_route = Route(unreachable_prefix, S_SPF, [], unreachable_negative_next_hops)
    rib.put_route(unreachable_route)
    unreachable_route_recover = Route(unreachable_prefix, S_SPF, [], ['S1', 'S2', 'S4'])
...
def test_add_subnet_disagg_recursive_unreachable():
    ...
    subnet_neg_route = Route(subnet_disagg_prefix, S_SPF, [], subnet_negative_disagg_next_hops)
    rib.put_route(subnet_neg_route)
    first_neg_unreach = Route(first_negative_disagg_prefix, S_SPF, [], unreachable_negative_next_hops)
...
def test_pos_neg_disagg():
    ...
    default_route = Route(default_prefix, S_SPF, default_next_hops)
    rib.put_route(default_route)
    leaf_route = Route(leaf_prefix, S_SPF, leaf_prefix_positive_next_hops, leaf_prefix_negative_next_hops)
...
def test_pos_neg_disagg_recursive():
    ...
    first_neg_route = Route(first_negative_disagg_prefix, S_SPF, [], first_negative_disagg_next_hops)
    rib.put_route(first_neg_route)
    subnet_neg_route = Route(subnet_disagg_prefix, S_SPF, ["S1"], subnet_negative_disagg_next_hops)
    rib.put_route(subnet_neg_route)
...
def test_add_two_route_same_destination():
    rib = Rib()
    default_route = Route(default_prefix, N_SPF, default_next_hops)
    rib.put_route(default_route)
    best_default_route = Route(default_prefix, S_SPF, ['S1'])
...
def test_add_two_route_same_destination_with_subnet():
    ...
    default_route = Route(default_prefix, N_SPF, default_next_hops)
    rib.put_route(default_route)
    best_default_route = Route(default_prefix, S_SPF, ["S1", "S2", "S3"])
    rib.put_route(best_default_route)
    first_neg_route = Route(first_negative_disagg_prefix, S_SPF, [], first_negative_disagg_next_hops)
    rib.put_route(first_neg_route)
    subnet_neg_route = Route(subnet_disagg_prefix, S_SPF, [], subnet_negative_disagg_next_hops)
    ...
    assert rib.fib.routes[first_negative_disagg_prefix] == first_neg_route
    assert rib.fib.kernel.routes[first_negative_disagg_prefix] == first_neg_route.next_hops
    # Test for 10.0.10.0/24
    assert rib.destinations.get(subnet_disagg_prefix).best_route == subnet_neg_route
    assert rib.destinations.get(subnet_disagg_prefix).best_route.positive_next_hops == set()
    assert rib.destinations.get(subnet_disagg_prefix).best_route.negative_next_hops == {'S2'}
