...
def test_pos_neg_disagg_recursive():
    ...
    new_subnet_disagg_next_hops = subnet_disagg_next_hops.copy()
    new_subnet_disagg_next_hops.append(('S1', DEST_TYPE_POS_DISAGG_PREFIX))
    subnet_disagg_route.add_next_hops(new_subnet_disagg_next_hops)
...
def test_add_two_route_same_destination():
    ...
    default_route.add_next_hops(default_next_hops)
    rib.put_route(default_route)
    best_default_route = Route(default_prefix, S_SPF)
    best_default_route.add_next_hops([('S1', DEST_TYPE_PREFIX)])
...
# test that the N_SPF is now preferred
...
def test_add_two_route_same_destination_with_subnet():
    ...
    rib.put_route(default_route)
    best_default_route = Route(default_prefix, S_SPF)
    best_default_route.add_next_hops([('S1', DEST_TYPE_PREFIX), ('S2', DEST_TYPE_PREFIX), ('S3', DEST_TYPE_PREFIX)])
    rib.put_route(best_default_route)
    first_neg_route = Route(first_negative_disagg_prefix, S_SPF)
    first_neg_route.add_next_hops(first_negative_disagg_next_hops)
    rib.put_route(first_neg_route)
    subnet_neg_route = Route(subnet_disagg_prefix, S_SPF)
    subnet_neg_route.add_next_hops(subnet_disagg_next_hops)
    rib.put_route(subnet_neg_route)
    assert rib.destinations.get(default_prefix).best_route == best_default_route
...
def test_remove_superfluous_subnet():
    rib = Rib()
    default_route = Route(default_prefix, S_SPF)
    default_route.add_next_hops(default_next_hops)
    rib.put_route(default_route)
    first_neg_route = Route(first_negative_disagg_prefix, S_SPF)
    first_neg_route.add_next_hops(first_negative_disagg_next_hops)
